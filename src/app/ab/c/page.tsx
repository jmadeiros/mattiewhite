...
// VERSION C: Tight spacing, dim + caption overlay in center, WITH zoom
export default function ABTestC() {
          ...
          <AnimatePresence mode="wait">
            {!showFullPortfolio ? (
                  ...
                  <div className="md:col-span-7 group cursor-pointer">
                    <ScrollReveal>
                      <div className="aspect-[4/5] overflow-hidden bg-stone-50 relative">
                        <Image src={portfolioImages[0].src || "/placeholder.svg"} alt={portfolioImages[0].alt} fill className="object-cover transition-transform duration-700 ease-out group-hover:scale-105" />
                      </div>
                    </ScrollReveal>
                  </div>
              ...
              <motion.div key="full" initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} transition={{ duration: 0.5 }} className="max-w-7xl mx-auto">
                {/* VERSION C: Tight spacing (gap-4), dim + caption overlay in center, WITH zoom */}
                    ...
                    {portfolioImages.map((item) => (
                      ...
                      >
                        <div className="aspect-[3/4] overflow-hidden bg-stone-50 relative">
                          {item.type === "video" ? (
                            <video src={item.src} title={item.alt} autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105" />
                          ) : (
                            <Image src={item.src || "/placeholder.svg"} alt={item.alt} fill className="object-cover transition-transform duration-700 ease-out group-hover:scale-105" />
                          )}
